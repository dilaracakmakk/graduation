...
namespace Graduation.WebUI.Management.Controllers
{
    ...
    public class ContentController : Controller
    {
        ...
        public ContentController(CategoryData categoryData, ContentCategoryData contentCategoryData,ContentData contentData, TagData tagData, ContentTagData contentTagData, MediaData mediaData)
        {
        ...
        {
            var content = new graduation.Model.Content();
            return View(content);
        }
        [HttpPost]
        public IActionResult Add(graduation.Model.Content content, List<int> CategoryIds, string Tags)
        {
            ...
            {
                var content_category_list = new List<graduation.Model.ContentCategory>();
                foreach (var cat in CategoryIds)
                {
                    content_category_list.Add(new graduation.Model.ContentCategory()
                    {
                        CategoryId = cat,
                ...
                if(!string.IsNullOrEmpty(Tags))
                {
                    ...
                    foreach(var tag in tag_split)
                    {
                        ...
                        {
                            var tag_model = new graduation.Model.Tag()
                            {
                                Name = name,
                                IsActive = true,
                                IsDeleted = false,
                            ...
                            var tag_content_model = new graduation.Model.ContentTag()
                            {
                                ContentId = content.Id,
                                TagId = tag_model.Id,
                            };
                ...
                return View(new graduation.Model.Content());
                }
        ...
        public IActionResult Edit(int id)
        {
            ...
            if (content == null)
                return RedirectToAction("Index", "Content", new { q = "makale bulunamadı" });
        ...
        [HttpPost]
        public IActionResult Edit(graduation.Model.Content content, List<int> CategoryIds, string Tags)
        {
            ...
            {
                viewModel.Content = content;
                ViewBag.Result = new ViewModelResult(false, "bu blug ile kayıt var");
                return View(viewModel);
            ...
            if(operationResult.IsSucceed)
            {
                if (!SetwiseEquivalentTo<int>(viewModel.Categories.Select(x => x.CategoryId).ToList(),CategoryIds))
                {
                    ...
                    foreach(var  cat in CategoryIds )
                    {
                        var cat_model = new graduation.Model.ContentCategory()
                        {
                            CategoryId = cat,
                            ContentId = content.Id,
                        };
                ...
                if (!string.IsNullOrEmpty(Tags) && !SetwiseEquivalentTo<string>(tags.Select(x => x.Name).ToList(), Tags.Split(',').Select(x => x.Trim()).ToList()))
                    ...
                    var content_tag_list = new List<graduation.Model.ContentTag>();
                    ...
                    foreach (var tag in tag_split)
                    {
                        ...
                        if(check_inDb_tag ==null)
                        {
                            ...
                            var insert_tag = _tagData.Insert(tag_model);
                            content_tag_list.Add(new graduation.Model.ContentTag()
                            {
                                ContentId = content.Id,
                                TagId = tag_model.Id,
                            });
        ...
        public IActionResult Delete(int id)
        {
            ...
            if (operationResult.IsSucceed)
                ...
                return RedirectToAction("Index", "Content", new { q = "makale-silinemedi" });

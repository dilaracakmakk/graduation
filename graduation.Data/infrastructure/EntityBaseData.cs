    ...
    public class EntityBaseData<T> : IData<T> where T : ModelBase
    {
        ...
        public DataResult DeleteByKey(int id)
        {
            try
            {
                ...
                if (aModel == null)
                    return new DataResult(false, "silinecek kayıt yok");
        ...
        public DataResult InsertBulk(List<T> ts, bool validateAndIgnoredBefore = false)
        {
            if (ts.Count <= 0)
                return new DataResult(true, "");
            try
            {
                ...
                {
                    if (validateAndIgnoredBefore && typeof(IValidatableObject).IsAssignableFrom(item.GetType()))
                    {
                        var results = new List<ValidationResult>();
                        bool İsValid = Validator.TryValidateObject(item, new ValidationContext(item, null, null), results, true);

                        if (!İsValid)
                        {
                            ValidationResult validationResult = results[0];
                            ValidationResult resultFirst = validationResult;
                        }
        ...
        public DataResult Update(T t)
        {
            try
            {
                ...
                if (aModel == null)
                    return new DataResult(false, "güncelleme yapılacak kayıt yok");
                BeforeUpdate();
